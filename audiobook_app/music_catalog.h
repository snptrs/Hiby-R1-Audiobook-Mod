...
typedef struct {
    ...
    int app_rows_removed;
} music_catalog_cleanup_result_t;

/* Remove entries under the app's own roots (/Audiobooks, /Podcasts) from
 * HiBy's stock Music databases. Missing DB paths are expected and are not
 * failures. Returns 0 when every existing DB was cleaned successfully, or -1
 * when at least one existing DB failed. */
